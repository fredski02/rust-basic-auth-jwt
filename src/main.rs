...
async fn main() {
    ...
    let routes = login_routes.or(user_route).or(admin_route).recover(error::handle_rejection);
...
fn init_users() -> HashMap<String, User> {
    ...
    map.insert("1".to_string(), User { uid : "1".to_string(), email : "johndoe-user@mail.com".to_string(), password : "test1".to_string(), role : "User".to_string()});
    map.insert("2".to_string(), User { uid : "2".to_string(), email : "johndoe-admin@mail.com".to_string(), password : "test1".to_string(), role : "Admin".to_string()});
